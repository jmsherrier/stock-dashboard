// package.json
{
  ...
  "version": "1.0.0",
  "description": "Momentum Trading Tracker Desktop App",
  ...
  "build": {
    "appId": "com.momentumtracker.app",
    "productName": "Momentum Tracker",
    "directories": {
      "output": "dist"
    },
    "files": [
...
function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const savedApi = saved.apiStatus || { daily: 0, minute: 0, lastReset: Date.now() };
      
      const today = new Date().toDateString();
      const lastResetDay = new Date(savedApi.lastReset).toDateString();
      if (today !== lastResetDay) {
        savedApi.daily = 0;
        savedApi.lastReset = Date.now();
      }
      
      setApiStatus(savedApi);
    }
    
    const backupInterval = setInterval(() => {
      if (stocks.length > 0) {
        storage.backup({ stocks, apiStatus });
      }
    }, 180000);

    const minuteReset = setInterval(() => {
      setApiStatus(prev => ({ ...prev, minute: 0 }));
    }, 60000);
