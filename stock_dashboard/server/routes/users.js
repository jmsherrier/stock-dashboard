...
router.delete('/clear-data', authenticateAPIKey, async (req, res) => {
  try {
    // Delete user stocks
    await global.db.run('DELETE FROM user_stocks WHERE user_id = ?', [req.user.id]);
    
    // Delete user settings
    await global.db.run('DELETE FROM user_settings WHERE user_id = ?', [req.user.id]);
    
    // Delete user strategies (if that table exists in the future)
    await global.db.run('DELETE FROM user_strategies WHERE user_id = ?', [req.user.id]);
